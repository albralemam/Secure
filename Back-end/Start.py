"""
Entry point for Secure for Windows.
...
Secure_PATH = os.path.join(current_file_path, 'iot-Secure-client')



Secure_URL = 'http://localhost:33761/Secure_dashboard'
GITHUB_REPO_URL = 'https://github.com/albralemam/Secure/tree/master'


def main():

    # Quit if Secure is already running
    try:
        urllib.request.urlopen(Secure_URL)
    except Exception:
        pass
    else:
        print('IoT Secure is already running. Aborted.')
        return
    ...
    if not os.path.exists(Secure_PATH):
        first_install = True
        print('Downloading the latest version of IoT Secure...')
        ...
        if os.path.isfile(os.path.join(current_file_path, 'DEBUG.txt')):
            sp.call([GIT_EXE_PATH, 'clone', '--depth', '1', '--branch', 'Secure2-dev', GITHUB_REPO_URL])
        else:
            sp.call([GIT_EXE_PATH, 'clone', '--depth', '1', GITHUB_REPO_URL])

    # Update the git repo by pulling
    os.chdir(Secure_PATH)
    ...
    proc = sp.Popen([
        PYTHON_PATH,
        '-m', 'pip', 'install', '-r',
        os.path.join(Secure_PATH, 'requirements.txt'),
        '--no-warn-script-location'
    ])
    ...
    if first_install:
        ...
        all_package_set = set()
        with open(os.path.join(Secure_PATH, 'requirements.txt'), 'r') as f:
            for line in f:
                line = line.strip()
                if line:
        ...
        while True:
            ...
            print('\n\nThis initial setup process will take several minutes, as we are preparing IoT Secure to run for the first time. Please be patient. If this process appears to be stuck, it is normal. Please do not close this window.\n')
    ...
    # Start Secure in a separte browser window
    print('Starting IoT Secure...')
    ui_dir = os.path.join(Secure_PATH, 'ui')
    sp.call(
        f"""cd "{ui_dir}" && powershell Start-Process -FilePath '{PYTHON_PATH}' -Verb RunAs -ArgumentList '-m streamlit run Device_List.py --server.port 33761 --browser.gatherUsageStats false --server.headless true --server.baseUrlPath Secure_dashboard'""",
        shell=True
    )

    # Wait for the Secure to start on the port
    for _ in range(60):
        print('Waiting for IoT Secure to start...')
        time.sleep(1)
        try:
            urllib.request.urlopen(Secure_URL)
            break
    ...
    print('IoT Secure started.')
    sp.call(f'start {Secure_URL}', shell=True)
